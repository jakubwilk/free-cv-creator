...
import { ActionIcon, AppShell, Box, Paper, ScrollArea, Skeleton, Stack } from '@mantine/core';
...
import { loadCvList, saveCvList } from '@dashboard/utils';
...
function persistCvData(data: CVData) {
  const list = loadCvList();
  const idx = list.findIndex((e) => e.id === data.meta.id);
  const entry = {
    id: data.meta.id,
...
export function EditorPage({ initialData }: EditorPageProps) {
  ...
  const { cvData, updatePersonal, updateSection, updateMeta } = useCvData(initialData);

  const saveTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  useEffect(() => {
    if (!mounted) return;
    if (saveTimerRef.current) clearTimeout(saveTimerRef.current);
    saveTimerRef.current = setTimeout(() => persistCvData(cvData), 600);
    return () => {
      if (saveTimerRef.current) clearTimeout(saveTimerRef.current);
    };
  }, [cvData, mounted]);
  ...
  const handleTemplateChange = (id: TemplateId) => {
    updateMeta({ templateId: id });
  };

  const handleCancel = () => router.push('/app');

  const handleDelete = () => {
    const list = loadCvList().filter((e) => e.id !== cvData.meta.id);
    saveCvList(list);
    router.push('/app');
  };
  ...
  const templateId = cvData.meta.templateId as TemplateId;
